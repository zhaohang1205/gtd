...
impl<'a> AppHandlers for App<'a> {
    ...
    fn confirm_input(&mut self, mode: Mode, input: &str) -> Result<()> {
        match mode {
            ...
            Mode::EditingTitle => self.confirm_edit_title(input)?,
            Mode::EditingDue => self.confirm_edit_due(input)?,
            Mode::EditingRrule => self.confirm_edit_rrule(input)?,
            Mode::EditingDelegated => self.confirm_edit_delegated(input)?,
...
impl<'a> App<'a> {
    ...
    fn confirm_edit_title(&mut self, input: &str) -> Result<()> {
    ...
    fn confirm_edit_due(&mut self, input: &str) -> Result<()> {
        if let Some(row) = self.items.get(self.selected).cloned() {
            let inp = input.trim();
            if inp.is_empty() {
    ...
    fn confirm_edit_rrule(&mut self, input: &str) -> Result<()> {
        if let Some(row) = self.items.get(self.selected).cloned() {
            let inp = input.trim();
    ...
    fn confirm_edit_delegated(&mut self, input: &str) -> Result<()> {
        if let Some(row) = self.items.get(self.selected).cloned() {
            let inp = input.trim();
